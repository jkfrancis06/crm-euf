<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <h3 class="col-lg-offset-5">Gestion des équipements<small></small></h3>
    </div>	
</div>
	...
	//		echo "<td>".$host["map"]."</td>";
		...
		//	 echo "<td>".$host["longitude_local"]."</td>";
			...
			if($host["isDown"]==1 || $host["isDown"]=='1'){
			 ...
			 echo "<a alt='Edition' title='Edition de HBS' href='".base_url()."index.php/bsu/edit?id=".$host["id"]."' class='btn btn-primary edit_btn' data-name='".$host["name"]."' data-id='".$host["id"]."' ><i class='fa fa-edit'></i></a>";
			  echo "<a alt='Suppression' title='Suppression de HBS' class='btn btn-danger del_btnbsu' data-name='".$host["name"]."' data-id='".$host["id"]."' data-toggle='modal' data-target='#myDeletebsu'><i class='glyphicon glyphicon-remove'></i></a>";
			 } else if($host["deviceTypeId"]==ASU_ID){
				 echo "<a alt='Edition' title='Edition de HSU' href='".base_url()."index.php/asu/edit?id=".$host["id"]."' class='btn btn-primary edit_btn' data-name='".$host["name"]."' data-id='".$host["id"]."' ><i class='fa fa-edit'></i></a>";
				 echo "<a alt='Suppression' title='Suppression de HSU'  class='btn btn-danger del_btn' data-name='".$host["name"]."' data-id='".$host["id"]."' data-toggle='modal' data-target='#myDelete'><i class='glyphicon glyphicon-remove'></i></a>";
			 }	else {
			echo "<a alt='Edition' title='Edition en HBS' href='".base_url()."index.php/bsu/edit?id=".$host["id"]."&changebsu=1' class='btn btn-primary edit_btn' data-name='".$host["name"]."' data-id='".$host["id"]."' > -->HBS<i class='fa fa-signal' aria-hidden='true'></i></a>";
